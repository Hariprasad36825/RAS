...
def create_user(request):
    if(request.method == 'POST'):
        ...
        try:
            ...
            if mat:
                new_user = Login(email=data["email"], password=hb.sha256(
                    data["password"].encode()).hexdigest(), type=data["type"].capitalize())
                new_user.save()
...
# print(my_custom_sql("select purchase.ingredient_id, inventory.name, purchase.quantity, purchase.price, cast(purchase.date as char) from purchase inner join inventory on purchase.ingredient_id = inventory.ingredient_id where purchase.date >= {}".format(
#     "2020-10-10")))
# purchase = Purchase.objects.filter(date__gte="2020-10-10")
# ans = list(purchase.values_list('ingredient_id', 'quantity', 'price', 'date'))
# for i, j in enumerate(ans):
#     tmp = list(ans[i])
#     tmp.insert(1, Inventory.objects.get(ingredient_id=j[0]).name)
#     tmp[-1] = tmp[-1].strftime("%Y-%m-%d %H:%M:%S")
#     ans[i] = tuple(tmp)
# print(ans)

def getPurchaseReport(date):
    purchase = Purchase.objects.filter(date__gte=date)
    ans = list(purchase.values_list(
        'ingredient_id', 'quantity', 'price', 'date'))
    for i, j in enumerate(ans):
        tmp = list(ans[i])
        tmp.insert(1, Inventory.objects.get(ingredient_id=j[0]).name)
        tmp[-1] = tmp[-1].strftime("%Y-%m-%d %H:%M:%S")
...
def getSalesReport(date):
    query = list(Sales.objects.values_list('item_code').annotate(
        quantity=Sum('quantity')).filter(date__gte=date))
    for j, i in enumerate(query):
        food = Food.objects.get(item_code=i[0])
        tmp = list(i)
        tmp.insert(1, food.name)
        tmp.append(i[1]*food.price)
        query[j] = tmp
    # print(query)
...
def getMonthlySales():
    ...
    ans = [0]*12
    sales = list(Sales.objects.values_list('item_code').annotate(
        quantity=Sum('quantity'), Month=TruncMonth('date')).filter(date__gte=datetime.now() - relativedelta(years=1)))
    for i in sales:
        food = Food.objects.get(item_code=i[0])
        # print()
        ans[int(i[-1].strftime("%m"))-1] += food.price*i[1]

    return ans


def getMonthlyPurchases():
    ans = [0]*12
    purchase = list(Purchase.objects.values_list('ingredient').annotate(
        quantity=Sum('price'), Month=TruncMonth('date')).filter(date__gte=datetime.now() - relativedelta(years=1)))
    for i in purchase:
        ans[int(i[-1].strftime("%m"))-1] += i[1]
    ...
    if(request.method == "POST"):
        try:
            ...
            if request.data['file'] == 'sales_report':
                query = getSalesReport(request.data["date"])
                ...
                data = pd.DataFrame(query, columns=[
                                    "Item Code", "Name", "Quantity", "Cost"])
                # print(data)
            elif request.data['file'] == 'purchase_report':
                ans = getPurchaseReport(request.data["date"])
                data = pd.DataFrame(ans, columns=[
                                    "Ingredient Id", "Ingredient name", "Quantity", "Price", "Purchase time"])

            else:
                ans = getSalesReport(request.data["date"])
                sales_data = pd.DataFrame(
                    ans, columns=["Item Code", "Food name", "Quatity", "Cost"])
                ans = getPurchaseReport(request.data["date"])
                purchase_data = pd.DataFrame(ans, columns=[
                                             "Ingredient Id", "Ingredient name", "Quantity", "Price", "Purchase time"])
                data = []
                ...
                data.append(("Loss", abs(gross_income))
                            if gross_income < 0 else ("Profit", gross_income))
...
def CreateExcel(request):
    ...
    if request.data['file'] == 'sales_report':
        ...
        monthly_purchase = getMonthlyPurchases()

        monthly_income = []
        for i in range(12):
            monthly_income.append(
                float(monthly_sales[i])-float(monthly_purchase[i]))
        ...
        data.append(("Total sales amount", total_sales))
        data.append(("Total purchase amount", total_purchase))
        data.append(("Loss", abs(gross_income)) if gross_income <
                    0 else ("Profit", gross_income))
        data = pd.DataFrame(data)
...
def Create_Pdf(request):
    ...
    customColor = colors.Color(
        red=(74.0/255), green=(98.0/255), blue=(255.0/255))
    customColor1 = colors.Color(
        red=(150.0/255), green=(180.0/255), blue=(255.0/255))
    customColor2 = colors.Color(
        red=(255.0/255), green=(255.0/255), blue=(255.0/255))
    ...
    if request.data['file'] == 'sales_report':
        ...
        # print(request.data['date'])
        # print("SELECT sales.item_code, food.name, sales.quantity, food.price*sales.quantity as COST , CAST(sales.date AS char) FROM food inner JOIN sales ON FOOD.ITEM_CODE = SALES.ITEM_CODE where date(sales.date) >= {}".format(request.data['date']))
        sales_data = getSalesReport(request.data['date'])
        ...
        elems.append(Paragraph("Sales report from "+datetime.strptime(request.data['date'], "%Y-%m-%d").strftime(
            "%d-%m-%Y"), ParagraphStyle("List", parent=getSampleStyleSheet()['Heading1'], alignment=1, spaceAfter=30)))
        elems.append(sales_table)

    elif request.data['file'] == 'purchase_report':
        purchase_data = getPurchaseReport(request.data['date'])
        purchase_head = ["Ingredient Id", "Ingredient name",
                         "Quantity", "Price", "Purchase time"]
        purchase_data.insert(0, purchase_head)
        purchase_table = Table(purchase_data)
        setBackground(purchase_table, len(purchase_data))
        elems.append(Paragraph("Purchase report from "+datetime.strptime(request.data['date'], "%Y-%m-%d").strftime(
            "%d-%m-%Y"), ParagraphStyle("List", parent=getSampleStyleSheet()['Heading1'], alignment=1, spaceAfter=30)))
        ...
        gross_income_data = []
        print(sum(sales_data['Cost']))
        total_sales = sum(sales_data['Cost'])
        total_purchase = sum(purchase_data['Price'])
        gross_income = total_sales - total_purchase
        ...
        gross_income_data.append(
            ("Loss", abs(gross_income)) if gross_income < 0 else ("Profit", gross_income))

        gross_income_table = Table(
            gross_income_data, colWidths=(4*inch, 1*inch))
        ...
        elems.append(Paragraph("Gross Income from "+datetime.strptime(request.data['date'], "%Y-%m-%d").strftime(
            "%d-%m-%Y"), ParagraphStyle("List", parent=getSampleStyleSheet()['Heading1'], alignment=1, spaceAfter=30)))
...
def get_chart_sales(request):
    ...
    if(request.method == 'POST'):
        ...
        # print(amt)
        x = list(Food.objects.filter(isvisible=1, sales__date__gte=datetime.now(
        ) - timedelta(days=30)).annotate(quan=Sum('sales__quantity')).values_list('name', 'quan'))
        amt = sum(list(Purchase.objects.filter(date__gte=datetime.now(
        ) - timedelta(days=30)).values_list('price', flat=True)))
        # print(x)
        color = []
        labels, data = zip(*x)
        labels = list(labels)
        ...
        l = len(data)
        while l > 0:
            col = 'rgba(' + str(random.randint(0, 200)) + "," + \
                str(random.randint(0, 200)) + "," + \
                str(random.randint(0, 200)) + ", 0.9)"
            if col not in color:
                color.append(col)
                l -= 1
            else:
...
def get_chart_purchase(request):
    ...
    if(request.method == 'POST'):
        ...
        # amt = my_custom_sql("select sum(price) from purchase where (DATE >= date(date('now','localtime'), '-1 month'))")
        # print(x)
        x = list(Inventory.objects.filter(purchase__date__gte=datetime.now(
        ) - timedelta(days=30)).annotate(quan=Sum('purchase__quantity')).values_list('name', 'quan'))
        amt = sum(list(Purchase.objects.filter(date__gte=datetime.now(
        ) - timedelta(days=30)).values_list('price', flat=True)))
        print(x)
        color = []
...
def GetFoodsForClerk(request):
    try:
        ...
        result = list(Food.objects.filter(Q(name__contains=request.data["val"]) | Q(
            item_code__contains=request.data["val"])).filter(isvisible=1).values_list('name', 'price', 'image', 'item_code'))
        # print(result)
        # print(result)
...
def get_foods(request):
    ...
    # result = list(my_custom_sql(query))
    result = list(Food.objects.filter(
        isvisible=1).values_list('item_code', 'name', 'price'))
    # print(result)
    return Response(result)
...
def getingredient_list(request):
    try:
        if(request.method == 'POST'):
            ...
            result = list([i[0]
                           for i in Inventory.objects.all().values_list('name')])
            # print(result)
            return Response(result)
        else:
...
def add_food(request):
    try:
        if(request.method == 'POST'):
            ...
            complementDict = dict(
                Food.objects.all().values_list('name', 'item_code'))
            ingredientsdict = dict(
                Inventory.objects.all().values_list('name', 'ingredient_id'))
            if Food.objects.filter(name=data["foodname"]).exists():
                ...
                for i in range(0, len(ingredientsList)):
                    ingredientsList[i] = str(
                        ingredientsdict[ingredientsList[i]])
                for i in range(0, len(complementaryList)):
                    if(len(complementaryList) != 0):
                        complementaryList[i] = str(
                            complementDict[complementaryList[i]])
                visibility = 1
                ...
                new_food = Food(name=data["foodname"].upper(), ingredient_list_id=ingListString, quantity_list=ingquantityString,
                                price=data["price"], isvisible=visibility, complementory_list=complementString)
                new_food.save()
                # print("finish")
...
def calculateThreshold(data):
    # print("ed")
    for item in data:
        ingredient_list, quantity_list, complementory_list = list(Food.objects.filter(
            pk=item[3]).values_list('ingredient_list_id', "quantity_list", "complementory_list"))[0]
        ...
        if (complementory_list):
            for complement in complementory_list.split(','):
                temp = list(Food.objects.filter(pk=int(complement)).values_list(
                    'ingredient_list_id', "quantity_list"))[0]
        ...
        for ingredient in range(len(ingredient_list)):
            if(DailyConsumption.objects.filter(ingredient_id=int(ingredient_list[ingredient]), date__exact=date.today()).exists()):
                temp = DailyConsumption.objects.get(ingredient_id=int(
                    ingredient_list[ingredient]), date__exact=date.today())

                temp.quantity = float(
                    temp.quantity) + float(item[1]) * float(quantity_list[ingredient])
                temp.save()
            else:
                DailyConsumption(ingredient_id=ingredient_list[ingredient], quantity=float(
                    item[1]) * float(quantity_list[ingredient])).save()

            threshold = DailyConsumption.objects.filter(ingredient_id=int(ingredient_list[ingredient]), date__gte=(
                (datetime.today() - timedelta(days=3)).strftime("%Y-%m-%d"))).values_list("quantity", flat=True)

            temp = Inventory.objects.get(
                ingredient_id=int(ingredient_list[ingredient]))
            temp.quantity = float(
                temp.quantity) - float(item[1]) * float(quantity_list[ingredient])
            ...
            if(temp.quantity < temp.thresholdvalue):
                ...
                if PurchaseList.objects.filter(ingredient_name=temp.name).exists():
                    if PurchaseList.objects.filter(ingredient_name=temp.name, is_ordered=1).exists():
                        ordered_list = PurchaseList.objects.get(
                            ingredient_name=temp.name, is_ordered=1)

                        try:
                            purchase_list = PurchaseList.objects.get(
                                ingredient_name=temp.name, is_ordered=0)
                            purchase_list.amount = (
                                temp.thresholdvalue - temp.quantity) - ordered_list.amount
                            purchase_list.save()
                        except:
                            # traceback.print_exc()
                            # print(ordered_list.amount)
                            PurchaseList(ingredient_name=temp.name, amount=(
                                (temp.thresholdvalue - temp.quantity) - ordered_list.amount)).save()
                    else:
                        purchase_list = PurchaseList.objects.get(
                            ingredient_name=temp.name)
                        purchase_list.amount = temp.thresholdvalue - temp.quantity
                        purchase_list.save()
                else:
                    PurchaseList(ingredient_name=temp.name, amount=(
                        temp.thresholdvalue - temp.quantity)).save()
                    # traceback.print_exc()
...
def get_purchase_list(request):
    if request.method == 'GET':
        try:
            ...
            pur_list = list(PurchaseList.objects.filter(
                is_ordered=0).values_list())
            # print(pur_list)
        ...
        try:
            ...
            for i in PurchaseList.objects.filter(is_ordered=0):
                ...
                if PurchaseList.objects.filter(ingredient_name=i.ingredient_name, is_ordered=1).exists():
                    purchase_list = PurchaseList.objects.get(
                        ingredient_name=i.ingredient_name, is_ordered=1)
...
def bill_generator(request):
    try:
        ...
        provider = Provider(
            'Restaurant', bank_account='2600420569', bank_code='2010')
        ...
        for item in data:
            ...
            Sales(item_code=Food.objects.get(
                item_code=int(item[3])), quantity=item[1]).save()
            balance += item[2]
...
def get_Invoice(request):
    if(request.method == 'POST'):
        ...
        # ingredientsdict=dict(my_custom_sql("select name,ingredient_id from inventory"))
        ingredientsdict = dict(
            Inventory.objects.filter().values_list('name', 'ingredient_id'))
        ...
        if(gndprice > actbal):
            ...
            for i in range(len(ingredientsList)):
                ...
                new_purchase = Purchase(
                    ingredient=ingred, quantity=quantityList[i], price=priceList[i], date=datetime.now())
                new_purchase.save()
                try:
                    purchase_list = PurchaseList.objects.get(
            ...
            for i in range(len(ingredientsList)):
                ...
                inventory = Inventory.objects.get(
                    ingredient_id=int(ingredientsList[i]))
                # print(inventory)
                inventory.quantity = float(
                    inventory.quantity) + quantityList[i]
                inventory.save()
            global invoice_no
...
def delete_food(request):
    if(request.method == 'POST'):
        ...
        try:
            ...
            food_row.delete()
            result = list(Food.objects.filter(isvisible=1).values_list(
                'item_code', 'name', 'price')[0])
            return Response(result)
        except:
            # traceback.print_exc()
