  ...
  const Complement = (props) => {
    return (
      <div>
        {props.complement.map((food, index) => {
          return (
            <div key={index} className="dropdowns form-field col-12">
              <label >
                complement:
                <select id={index} value={props.complement[index]} onChange={(event)=>props.handleComplementName(event,index)}>
        ...
        <button className="add-btn" type="button" onClick={(e) => props.addComplement(e)}>
          add
        </button>
        </div>
        <div className="form-field col-12">
